...
### R1. Ухвалити скриптовану механіку рушія для 2–3 ключових політій
...
нижчий (ціна — DDS-арт бару, рамки, кнопки й стрічка alert-іконок).

Кандидати за спаданням цінності:
...
| **Київ: Сеніорат / Рота** | `04_russian` (кнопки) | кнопка «Передати уділ» (вже є як дипдія `rip_grant_the_udil`), кнопка «Скликати з'їзд князів», бар `seniority` — легітимність старшинства проти удільної відцентровості |

Не робити всі одразу. **Січ і Гетьманщина** — найбільша віддача: обидві
центральні для мода, обидві зараз фейкаються, обидві мають чисту фікцію
(«Рада обирає кошового», «старшина захоплює булаву»).
...
1. Скриптована механіка рушія (R1) — так чи ні, і для яких політій спершу
   (рекомендація: Січ + Гетьманщина)?
